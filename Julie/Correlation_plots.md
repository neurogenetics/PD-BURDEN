...
variant_group <- unique(Mrg$VARIANT_GROUP.y)

if (variant_group == "ALL_MISSENSE_SNPEFF") {
    variant_group_plt <- "Missense (Annovar vs. SnpEff)"
    y_axis <- expression("log"[10]*" SnpEff P-value")
} else if (variant_group == "ALL_MISSENSE_and_LOF_SNPEFF") {
    variant_group_plt <- "Missense and LOF (Annovar vs. SnpEff)"
    y_axis <- expression("log"[10]*" SnpEff P-value")
} else if (variant_group == "ALL_MISSENSE_and_LOF_SNPEFF_and_HC_LOFTEE") {
    variant_group_plt <- bquote(bold("Missense and LOF (Annovar vs. SnpEff"~intersect(LOFTEE)*")"))
    y_axis <- expression("log"[10]*" SnpEff / LOFTEE P-value")
} else if (variant_group == "ALL_MISSENSE_and_ALL_LOF_HC_LOFTEE") {
    variant_group_plt <- "Missense (Annovar vs. SnpEff) and LOF (Annovar vs. LOFTEE)"
    y_axis <- expression("log"[10]*" SnpEff / LOFTEE P-value")
} else if (variant_group == "ALL_LOF_SNPEFF") {
    variant_group_plt <- "LOF (Annovar vs. SnpEff)"
    y_axis <- expression("log"[10]*" SnpEff P-value")
} else if (variant_group == "ALL_LOF_HC_LOFTEE") {
    variant_group_plt <- "LOF (Annovar vs. LOFTEE)"
    y_axis <- expression("log"[10]*" LOFTEE P-value")
} else if (variant_group == "ALL_LOF_and_HC_LOFTEE") {
    variant_group_plt <- bquote(bold("LOF (Annovar vs. SnpEff"~intersect(LOFTEE)*")"))
    y_axis <- expression("log"[10]*" SnpEff / LOFTEE P-value")
} else if (variant_group == "ALL_MODERATE_HIGH_IMPACT_SNPEFF") {
    variant_group_plt <- "CADD > 10 (Annovar) vs. Moderate or High Impact (SnpEff)"
    y_axis <- expression("log"[10]*" SnpEff P-value")
} else if (variant_group == "ALL_HIGH_IMPACT_SNPEFF") {
    variant_group_plt <- "CADD > 20 (Annovar) vs. High Impact (SnpEff)"
    y_axis <- expression("log"[10]*" SnpEff P-value")
} else if (variant_group == "ALL_HIGH_IMPACT_and_LOF_SNPEFF") {
    variant_group_plt <- "CADD > 20 and LOF (Annovar) vs. High Impact and LOF (SnpEff)"
    y_axis <- expression("log"[10]*" SnpEff P-value")
} else if (variant_group == "ALL_HIGH_IMPACT_and_LOF_SNPEFF_and_HC_LOFTEE") {
    variant_group_plt <- bquote(bold("CADD > 20 and LOF (Annovar) vs. High Impact and LOF (SnpEff"~intersect(LOFTEE)*")"))
    y_axis <- expression("log"[10]*" SnpEff / LOFTEE P-value")
} else if (variant_group == "ALL_HIGH_IMPACT_and_ALL_LOF_HC_LOFTEE") {
    variant_group_plt <- "CADD > 20 and LOF (Annovar) vs. High Impact (SnpEff) and LOF (LOFTEE)"
    y_axis <- expression("log"[10]*" SnpEff / LOFTEE P-value")
}
...
plt_title <- bquote(bold(.(TEST)~"Burden Test with MAF" <= .(MAF)~"and"~N[variants~per~gene] >= .(MIN_NUMVAR)))
...
pdf(paste(OUTNAME, ".pdf", sep=""), onefile=F, width=8, height=8)
plot.new()
ggscatter(Mrg, x = "META1_logP", y = "META2_logP", add = "reg.line", 
xlab = expression("log"[10]*" Annovar P-value"), ylab = y_axis, 
add.params = list(color = "blue"), color="gray") + 
theme(plot.margin =margin(5.5, 1, 1, 1, "cm"))

# Add titles to plot
mtext(side=3, line = 1.5, "Meta-analysis of AMP-PD x NIH WGS and UK Biobank WES", cex=1, font=2)
mtext(side=3, line = 0, meta_group, cex=0.8, font=2)
mtext(side=3, line = -1.5, variant_group_plt, cex=0.8, font=2)
mtext(side=3, line = -3, plt_title, cex=0.8, font=2)

# Add correlations of the meta-analyses
cor1 <- round(cor(Mrg$META1_logP,Mrg$META2_logP, use ="complete.obs"), 2)
mtext(side=3, line = -4.7, bquote(R[Meta] == .(cor1)), cex=1, font=3, col="red")
...
if (meta_group == "Cases vs. Controls") {
    ...
    mtext(side=3, line = -6, bquote(R[AMP~x~NIH] == .(cor2)~~~R[UKB] == .(cor3)), cex=0.8, font=1)
    ...
    mtext(side=3, line = -6, bquote(R[AMP~x~NIH] == .(cor2)~~~R[UKB~Case] == .(cor3)), cex=0.8, font=1)
    mtext(side=3, line = -7, bquote(R[UKB~Sibling] == .(cor4)~~~R[UKB~Parent] == .(cor5)), cex=0.8, font=1)
} else {
    cor3 <- round(cor(-log(Mrg$UKB_ALL_CASES_PVAL.x, base=10), -log(Mrg$UKB_ALL_CASES_PVAL.y, base=10), use ="complete.obs"), 2)
    mtext(side=3, line = -6, bquote(R[AMP~x~NIH] == .(cor2)~~~R[UKB~All~Phenotypes] == .(cor3)), cex=0.8, font=1)
}
dev.off()
```
