...
# Meta-analyses script for burdens
    ...
    # Takes in .assoc files generated by RVTests
        # python cmcwald_weightedZ_gne_AMPNIH_UKB_CASES_PROXIES_meta_analysis_numvar.py \
        #             --test [CMCWald] \
        #             --numvar [minimum number of variants per gene you want to keep] \
        #             --ntotal [int; total number of cases+controls to adjust for] \    
...
parser.add_argument("--ntotal", type=int, help="Input number of total cases+controls")
...
if test == "CMCWald":
    ...
    ukb_sib_raw_df.drop(columns=['RANGE','N_INFORMATIVE','NumVar','NumPolyVar','NonRefSite'], inplace=True)
    ukb_parent_raw_df.drop(columns=['RANGE','N_INFORMATIVE','NumVar','NumPolyVar','NonRefSite'], inplace=True)
...
original_pval_df.columns = ['GENE', 
                            'UKB_CASE_BETA', 'UKB_CASE_SE', 'UKB_CASE_PVAL',
                            'AMP_NIH_BETA', 'AMP_NIH_SE', 'AMP_NIH_PVAL',
                            'UKB_SIBLING_PROXY_BETA', 'UKB_SIBLING_PROXY_SE', 'UKB_SIBLING_PROXY_PVAL', 
                            'UKB_PARENT_PROXY_BETA', 'UKB_PARENT_PROXY_SE', 'UKB_PARENT_PROXY_PVAL',
...
# Make variable to pass through
ntotal = args.ntotal

## Loop it using weighted Z approach (to account for directionality and limiting potential genomic inflation)
results = []
...
final_dataframe['NTOTAL'] = args.ntotal
