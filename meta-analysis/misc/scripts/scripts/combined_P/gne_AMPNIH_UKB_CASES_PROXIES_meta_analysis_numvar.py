...
# Meta-analyses script for burdens
    ...
    # Takes in .assoc files generated by RVTests
        # python gne_AMPNIH_UKB_CASES_PROXIES_meta_analysis_numvar.py \
        #             --test [SkatO, CMCWald] \
        ...
        #             --ukb_sib [filepath for proxies #1 .assoc file - suffix included] \
        #             --ukb_parent [filepath for proxies #2 .assoc file - suffix included] \
...
parser.add_argument("--ukb_sib", help="Input file name (with suffix) for UKB proxy siblings .assoc")
parser.add_argument("--ukb_parent", help="Input file name (with suffix) for UKB proxy parents .assoc")
...
if test == "SkatO": 
    ...
    ukb_sib_raw_df = pd.read_csv(args.ukb_sib, delim_whitespace=True, header=0, names=header_text)
    ukb_parent_raw_df = pd.read_csv(args.ukb_parent, delim_whitespace=True, header=0, names=header_text)
    
    ukb_cases_raw_df['Gene'] = ukb_cases_raw_df['Gene'].astype(str)
    ukb_sib_raw_df['Gene'] = ukb_sib_raw_df['Gene'].astype(str)
    ukb_parent_raw_df['Gene'] = ukb_parent_raw_df['Gene'].astype(str)
    amp_nih_raw_df['Gene'] = amp_nih_raw_df['Gene'].astype(str)

    gne_header_text = ['Gene', 'NumVar', 'Pvalue']
    ...
    ukb_sib_raw_df = ukb_sib_raw_df[ukb_sib_raw_df['NumVar'] >= args.numvar]
    ukb_parent_raw_df = ukb_parent_raw_df[ukb_parent_raw_df['NumVar'] >= args.numvar]

    gne_df = gne_df[gne_df['NumVar'] >= args.numvar]

    # Save the NumVars to merge with later...
    ukb_numvar = ukb_cases_raw_df[['Gene', 'NumVar']].copy()
    ...
    ukb_sib_raw_df.drop(columns=['RANGE','N_INFORMATIVE','NumVar','NumPolyVar','Q','rho'], inplace=True)
    ukb_parent_raw_df.drop(columns=['RANGE','N_INFORMATIVE','NumVar','NumPolyVar','Q','rho'], inplace=True)
    ...
    # Drop the unncessary columns, leaving just GENE and P-VALUE
    print(f"Dropping unneccesary columns...")
    ukb_cases_raw_df.drop(columns=['RANGE','N_INFORMATIVE','NumVar','NumPolyVar','NonRefSite','BETA','SE'], inplace=True)
    amp_nih_raw_df.drop(columns=['RANGE','N_INFORMATIVE','NumVar','NumPolyVar','NonRefSite','BETA','SE'], inplace=True)
    ukb_sib_raw_df.drop(columns=['RANGE','N_INFORMATIVE','NumVar','NumPolyVar','NonRefSite','BETA','SE'], inplace=True)
    ukb_parent_raw_df.drop(columns=['RANGE','N_INFORMATIVE','NumVar','NumPolyVar','NonRefSite','BETA','SE'], inplace=True)

    gne_df.drop(columns=['NumVar', 'BETA', 'SE'], inplace=True)
...
data_frames = [ukb_cases_raw_df, amp_nih_raw_df, ukb_sib_raw_df, ukb_parent_raw_df, gne_df]

original_pval_df = ukb_cases_raw_df.merge(amp_nih_raw_df,on='Gene', how="outer").merge(ukb_sib_raw_df, on='Gene', how="outer").merge(ukb_parent_raw_df,on='Gene', how="outer").merge(gne_df,on='Gene', how="outer").fillna("NA")
original_pval_df.columns = ['GENE', 
                            'UKB_CASE_PVAL', 
                            'AMP_NIH_PVAL',
                            'UKB_SIBLING_PROXY_PVAL', 
                            'UKB_PARENT_PROXY_PVAL',
                            'GNE_PVAL']
...
total_numvars = ukb_numvar.merge(amp_nih_numvar, on='GENE', how="outer").merge(ukb_sib_numvar, on='GENE', how="outer").merge(ukb_parent_numvar, on='GENE', how="outer").merge(gne_numvar, on='GENE', how="outer").fillna("NA")
merge_pvals.GENE.astype(str)
total_numvars.GENE.astype(str)
