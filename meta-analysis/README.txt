# README by Mary Makarious; Last Updated 16-FEB-2022
	
GitHub Directories 
- meta_ampnih_gne_ukb: AMPxNIH + UKB + Genentech included in discovery
- meta_ampnih_ukb: AMPxNIH + UKB_ALL only included in discovery
	...
	# PD Burdens meta-analysis with AMPxNIH, Genentech, and UKB cohorts 
		# Working directory on Biowulf: /data/CARD/PD/AMP_NIH/no_relateds/meta_risk_analysis/geneburden_risk_wGenentech/
			# ├── AMP_NIH_UKB_LAMBDA_SUMMARIES/
			# ├── AMPxNIH/
			# ├── geneburden_meta_genentech_data.sh
			# ├── GENENTECH/
			# ├── LAMBDA_SUMMARIES/
			# ├── manhattan_burden_1e-6.R
			# ├── manhattan_burden.R
			# ├── README.txt
			# ├── RESULTS_COMBINED_P/
	...
	# Results broken by minimum number of variants 
		# Combined p-values method (Fisher) for SkatO and CMC Wald: /data/CARD/PD/AMP_NIH/no_relateds/meta_risk_analysis/geneburden_risk_wGenentech/RESULTS_COMBINED_P/
			# Broken up by cohort (5 in total) per directory
				# minimum_numvar_* : sub-directories broken up by minimum number of variants 
			# SUMMARIES/ : directory where top 25 meta-pval in all cohorts with AMPxNIH + UKB + Genentech included in discovery
			# AMP_NIH_UKB_SUMMARIES/: : directory where top 25 meta-pval in all cohorts with AMPxNIH + UKB_ALL only included in discovery
			# LAMBDAS/ : directory where lambda and lambda 1000 values were calculated 
		
		# Summed Z for CMC Wald: /data/CARD/PD/AMP_NIH/no_relateds/meta_risk_analysis/geneburden_risk_wGenentech/RESULTS_SUMMEDZ_CMCWALD/
			# Broken up by cohort (5 in total) per directory
				# minimum_numvar_* : sub-directories broken up by minimum number of variants 
			# SUMMARIES/ : directory where top 25 meta-pval in all cohorts with AMPxNIH + UKB + Genentech included in discovery
			# AMP_NIH_UKB_SUMMARIES/: : directory where top 25 meta-pval in all cohorts with AMPxNIH + UKB_ALL only included in discovery
		...
		# Weighted Z for CMC Wald: /data/CARD/PD/AMP_NIH/no_relateds/meta_risk_analysis/geneburden_risk_wGenentech/RESULTS_WEIGHTEDZ_CMCWALD/
			# Broken up by cohort (5 in total) per directory
				# minimum_numvar_* : sub-directories broken up by minimum number of variants 
			# SUMMARIES/ : directory where top 25 meta-pval in all cohorts with AMPxNIH + UKB + Genentech included in discovery
			# AMP_NIH_UKB_SUMMARIES/: : directory where top 25 meta-pval in all cohorts with AMPxNIH + UKB_ALL only included in discovery
		...
		# LAMBDA SUMMARIES:
			# LAMBDA_SUMMARIES/ for meta-analysis with AMPxNIH + UKB + Genentech included in discovery
			# AMP_NIH_UKB_LAMBDA_SUMMARIES/ for AMPxNIH + UKB_ALL only in discovery (to use GNE as replication)
		
		# MANHATTAN PLOTS:
			# Within each results directory; labeled MANHATTANS/ or AMP_NIH_UKB_ALL_PD_PHENO_MANHATTANS/
	...
	# Cohorts
		...
		# AMP_NIH_UKB_ALL_PD_PHENO_META
			# AMPxNIH WGS vs. UKB PD case-control+proxies 
