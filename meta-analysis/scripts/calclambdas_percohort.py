...
# Meta-analyses script for burdens - calculate lambdas
    ...
    # Start script like this:
        # python calclambdas_percohort.py \
        #             --test [SkatO, CMCWald] \
...
parser.add_argument("--pval", type=str, help="Input string with what column name has p-value")
parser.add_argument("--numvar_col", type=str, help="Input string with what column name has NumVar column")
...
pval = args.pval
numvar_col = args.numvar_col

# Subset by numvar
input_df = input_df[input_df[numvar_col] >= args.numvar]

# Save out just pvals 
df = input_df[[pval]].copy()
...
# Calculate values 
lambda_val = calculate_inflation(df[pval])
lambda1000_val = calculate_inflation(df[pval], normalize=True, ncases=args.ncases, ncontrols=args.ncontrols)
...
output = pd.DataFrame(results, columns=['COHORT', 'VARIANT_GROUP', 'MAF', 'TEST', 'MIN_NUMVAR', 'LAMBDA', 'LAMBDA1000'])
