...
# Meta-analyses script for burdens
    ...
    # Takes in .assoc files generated by RVTests
        # python gne_AMPNIH_UKB_meta_analysis_numvar.py \
        #             --test [SkatO, CMCWald] \
...
data_frames = [ukb_cases_raw_df, amp_nih_raw_df, gne_df]
original_pval_df = reduce(lambda left,right: pd.merge(left,right,on=['Gene'], how='outer'), data_frames).fillna('NA')
original_pval_df.columns = ['GENE', 'UKB_CASE_PVAL', 'AMP_NIH_PVAL', 'GNE_PVAL']
...
total_numvars = ukb_cases_numvar.merge(amp_nih_numvar, on='GENE', how="outer").merge(gne_numvar, on='GENE', how="outer").fillna("NA")
